...
begin
  ...
  CloseFile(f);
  Writeln('Загружено ', i, ' записей из File4.txt');
end;

// Аналогично для LoadFile2
...
begin
  ...
  begin
    ...
    begin
      Writeln('Проверка: ', Trim(People1[i].LastName), ' и ', Trim(People2[j].LastName));
  ...
  Writeln('Количество объединенных записей: ', CombinedCount);
end;
...
begin
  ...
  begin
    Writeln('Фамилия: ', CombinedPeople[x].LastName,
            ' Направление: ', CombinedPeople[x].Direction,
            ' Год рождения: ', CombinedPeople[x].BirthYear,
            ' Страна: ', CombinedPeople[x].Country,
            ' Место рождения: ', CombinedPeople[x].PlaceBirth);
  end;
end;
